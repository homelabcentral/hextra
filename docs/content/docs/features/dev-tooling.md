...
- **`dev`** — the Go devcontainer image your editor attaches to, with the repo mounted at `/workspaces/hextra`. Devcontainer features install Hugo Extended (pinned version), Node 24, the Docker CLI against the host daemon, act, and the GitHub CLI; `postCreateCommand` runs `npm install` so the container is build-ready on first open, then installs two content-authoring tools: [**freeze**](https://github.com/charmbracelet/freeze), which renders code snippets to SVG with the font embedded, and **asciinema**, which records the `.cast` files the [asciinema shortcode](../guide/shortcodes/asciinema) plays. A curated set of VS Code extensions (Tailwind, Hugo, Prettier, Git Graph, …) comes preconfigured.
