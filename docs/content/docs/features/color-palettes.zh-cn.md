...
| 浮层——下拉框、折叠块   | `neutral-100` | `neutral-900` |
| 外框——文件名栏、悬停   | `neutral-200` | `neutral-800` |
| 边框                   | `neutral-300` | `neutral-700` |

注意方向在两种模式之间是相反的：浅色模式下抬升表面比页面 _更亮_，深色模式下则 _更暗_。

折叠块——手风琴、系列盒、`details` 短代码——不再自成一级，而是落在浮层上。决定层级的是元素装着什么：凡是承载正文的都和代码块一起放在抬升表面，博客卡片、文章卡片和仓库卡片都是如此；而封面或缩略图所在的槽位要再上一级到浮层，这样没有填满整列的图片仍然读作高于卡片的表面。

边框只有一对，1px，不分容器——代码块、卡片、折叠块还是命令框都一样。悬停时边缘在浅色模式抬到 `neutral-500`，深色模式抬到 `neutral-600`。
